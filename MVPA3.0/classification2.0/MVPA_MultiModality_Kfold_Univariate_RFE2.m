...
%       扩展到2组多个模态，2018-04-19 By Lichao
%       将opt移到最后。2018-06-25 By Lichao
%% set options
...
if nargin<2 && ~exist('dataMat','var')
    ...
    path=folderPath{1};
    [dataMatCell,nameFirstModality]=dataCell2Mat(dataCell,fileName,opt.numOfFeatureType,opt.loadOrderOfData);% 按特征分配到各个cell,顺序有调整
...
mean_brain_weight=zeros(numOfFeatureSet,dim1*dim2*dim3);% 目前仅适用于单模态
brain_weight=zeros(numOfFeatureSet, size(data_inmask,2),opt.K);% 目前仅适用于单模态
...
for i=1:opt.K
    ...
    % step3：加入单变量的特征过滤
    [~,P,~,~]=ttest2(dataTrain(labelTrain==1,:), dataTrain(labelTrain==2,:),'Tail','both');% 目前仅适用与单模态
...
if opt.weight
    W_mean=AverageWeightMap(brain_weight,opt.percentage_consensus);% W_mean的维度和maskForFilter一致。
    %     W_M_Brain=data2originIndex({find(maskForFilter~=0)},W_mean,length(maskForFilter));
    mean_brain_weight(:,maskForFilter~=0)=W_mean;%不同feature 数目时的全脑体素权重
end
% 计算最佳的分类性能
